...
class EYEPlayerController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // 开始播放
        self.player.play()
        self.playView.startBtn.selected = true
    ...
    private func addObserverAndNotifacation() {
        ...
        // 播放按钮点击事件
        self.playView.startBtn.addTarget(self, action: #selector(EYEPlayerController.startAction(_:)), forControlEvents: .TouchUpInside)
        // 返回按钮点击事件
    ...
    @objc private func moviePlayDidEnd(notification: NSNotification) {
        self.state = .PlayerStateStopped
        self.playView.startBtn.selected = false
    }
    ...
    @objc private func progressSliderValueChanged(slider : UISlider) {
        ...
        if self.player.status == .ReadyToPlay {
            ...
            if durSec > 0 {
                // 当总时长>0时候才能拖动slider
                self.playView.startTime.text = currentTime
...
//        self.addObserverAndNotifacation()
    ...
    @objc private func playerTimerAction() {
        ...
        self.playView.startTime.text = String(format: "%02zd:%02zd", proMin,proSec)
        self.playView.endTime.text = String(format: "%02zd:%02zd", durMin, durSec)
    }
