...
class EYEChoiceController: EYEBaseViewController, LoadingPresenter, MenuPresenter {
    ...
    override func viewDidLoad() {
        ...
        setupLoaderView()
        // 获取数据
        getData(FJAPIHeapler.API_Choice, params: ["date" : NSDate.getCurrentTimeStamp(), "num" : "7"])
        setLoaderViewHidden(false)
        // 初始化菜单按钮
        setupMenuBtn()
        // 下拉刷新
        collectionView.headerViewPullToRefresh { [unowned self] in
            self.getData(FJAPIHeapler.API_Choice, params: ["date": NSDate.getCurrentTimeStamp(), "num": "7"])
        }
        // 上拉加载更多
    ...
    private func getData(api: String, params: [String : AnyObject]? = nil) {
        Alamofire.request(.GET, api, parameters: params).responseSwiftyJSON ({[unowned self](request, Response, json, error) -> Void in
            print("\(FJAPIHeapler.API_Choice)- \(params)")
...
extension EYEChoiceController : UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        let cell : EYEChioceCell = collectionView.dequeueReusableCellWithReuseIdentifier(EYEChioceCell.reuseIdentifier, forIndexPath: indexPath) as! EYEChioceCell
        
        return cell
    }
    
    func collectionView(collectionView: UICollectionView, didSelectItemAtIndexPath indexPath: NSIndexPath) {
        self.selectCell = collectionView.cellForItemAtIndexPath(indexPath) as! EYEChioceCell
        
        let issueModel = issueList[indexPath.section]
        let model: ItemModel = issueModel.itemList[indexPath.row]
        // 如果播放地址为空就返回
        if model.playUrl.isEmpty {
            APESuperHUD.showOrUpdateHUD(icon: .SadFace, message: "没有播放地址", duration: 0.3, presentingView: self.view, completion: nil)
            return
        }
